...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!fullName || !email || !role) return NextResponse.json({ error: "Full name, email, and role required." }, { status: 400 });

    const { databases } = createAdminClient();
    const existing = await databases.listDocuments(DB_ID, COLLECTIONS.CHALLENGE_WAITLIST, [Query.equal("email", email.toLowerCase().trim()), Query.limit(1)]);
    if (existing.total > 0) return NextResponse.json({ error: "Already on the waitlist!" }, { status: 409 });
    ...
    await databases.createDocument(DB_ID, COLLECTIONS.CHALLENGE_WAITLIST, "unique()", {
      ...
      role, buildIdea: buildIdea?.trim() || null, country: country?.trim() || null, referralCode: refCode,
    });

    const all = await databases.listDocuments(DB_ID, COLLECTIONS.CHALLENGE_WAITLIST, [Query.limit(1)]);
...
export async function GET() {
  try {
    const { databases } = createAdminClient();
    const result = await databases.listDocuments(DB_ID, COLLECTIONS.CHALLENGE_WAITLIST, [Query.limit(1)]);
    return NextResponse.json({ totalSignups: result.total });
  } catch { return NextResponse.json({ totalSignups: 0 }); }
}
